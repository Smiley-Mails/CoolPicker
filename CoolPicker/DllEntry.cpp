...
//Cool Picker specific header filesЫ
#include "initialize_coolpick_windows.h"

extern ClassDesc2* GetCoolPickerDesc();
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL,ULONG fdwReason,LPVOID lpvReserved)
{
	if( fdwReason == DLL_PROCESS_ATTACH )
	{
		...
		if (!controlsInit) {
			...
			InitCoolPickerControls(hInstance);		// Initialize the new custom Color Stuff!!! [See Controls.cpp]
		}
...
{
	switch(i) {
	case 0: return GetCoolPickerDesc();
	default: return 0;
	}
}
